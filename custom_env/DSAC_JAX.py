...
from sbx.common.type_aliases import RLTrainState
import time
from discrete_policy import DiscreteDistributionalVectorCritic
...
class DSAC_JAX(SAC):
    ...
    def __init__(self, policy, env, env_rec, latency_manager: JaxLatencyEnv, latency_manager_test: JaxLatencyEnv,buffer : CustomBufferBis, learning_rate = 0.0003, 
        ...
        self.start_std, self.start_entropy = jnp.ones((2)), 0.5 * target_entropy
    ...
    def rollout_warmup(cls, env: EnvDataBinance, n_episodes, n_networks_training, actor_state, qf_state, act_dim, tau, batch_size, past_entropy, obs_dim, past_std,
        ...
        return env, actor_state, qf_state, past_std, entropy, rec_losses
    ...
    def learning_wrapper(cls, total_timesteps, collect_timesteps, buffer_shape, env, eval_env, buffer: CustomBufferLatency, obs_dim, act_dim, gradient_steps, discounts,
                                 ...
                                 key: jax.Array, init_timesteps, eval_freq: int, n_eval: int, log_interval: int, latency_manager: JaxLatencyEnv, latency_manager_test: JaxLatencyEnv, start_entropy, start_std,
        ...
        env, actor_state, qf_state, start_std, start_entropy, rec_losses = cls.rollout_warmup(env, n_episodes_warmup, n_networks_training_warmup, actor_state, 
    ...
    def learn_jax(
        ...
        carry, loss_warmup = self.learning_wrapper(total_timesteps, self.train_freq.frequency, self.buffer.buffer.shape[1], self.env, self.env_rec, self.buffer, self.env.obs_length, self.env.action_length, self.gradient_steps, self.gamma,
                                 ...
                                 eval_freq, n_eval, log_interval, self.latency_manager, self.latency_manager_test, self.start_entropy, self.start_std, self.n_episodes_warmup, self.n_training_warmup)
    ...
    @staticmethod
    @partial(jax.jit, static_argnames = ["action_dim", "tau"])
    def update_critic(
        ...
        std_next = std_next.reshape(2, -1, action_dim)

        C1 = ent_coef_value * log_prob # (1)
        ...
        target_Z_values = jnp.min(normal_values, axis=0) # (B, |A|)
        target_Z_values = (prob * (target_Z_values - C1)).sum(axis = -1) #(B, T)
        target_Z_values = jax.lax.stop_gradient(rewards+ C2 * target_Z_values) #(B)
        ...
        def mse_loss(params: flax.core.FrozenDict, dropout_key: jax.Array) -> jax.Array:
            ...
            observed_std = current_std_values[:,jnp.arange(actions.shape[0]), actions.astype(jnp.int32).squeeze(-1)] #(2, B)

            new_means_std = (1 - tau) * past_std + tau *  jax.lax.stop_gradient(observed_std).mean(axis = 1) # (2, |A|)

            td_bound_1 = 3 *  jax.lax.stop_gradient(new_means_std[0]) # (|A|)
            td_bound_2 = 3 *  jax.lax.stop_gradient(new_means_std[1]) # (|A|)

            difference_1 = jnp.clip(jax.lax.stop_gradient(target_Z_values) -  jax.lax.stop_gradient(observed_q_values[0]), -td_bound_1, td_bound_1) # (B,|A|)
            difference_2 = jnp.clip(jax.lax.stop_gradient(target_Z_values) -  jax.lax.stop_gradient(observed_q_values[1]), -td_bound_2, td_bound_2) # (B,|A|)

            target_q_bound_1 =  jax.lax.stop_gradient(observed_q_values[0]) + difference_1 # (B,|A|)
            target_q_bound_2 =  jax.lax.stop_gradient(observed_q_values[1]) + difference_2 # (B,|A|)
            
            q1_std = observed_std[0] # (B, |A|)
            q2_std = observed_std[1] # (B, |A|)
            ...
            ratio1 = jnp.clip(new_means_std[0] ** 2 / (q1_std_detach ** 2 + bias), 0.1,10) # (B, |A|)
            ratio2 = jnp.clip(new_means_std[1] ** 2 / (q2_std_detach ** 2 + bias), 0.1,10) # (B, |A|)

            L_mean_1 = ratio1 * optax.losses.huber_loss(observed_q_values[0] , target_Q_values, 50) # (B)
            L_mean_2 = ratio2 * optax.losses.huber_loss(observed_q_values[1] , target_Q_values, 50) # (B)
            
            L_var_1 = ratio1 * q1_std / (q1_std_detach + bias) * (q1_std_detach**2 - optax.losses.huber_loss( jax.lax.stop_gradient(observed_q_values[0]), target_q_bound_1, 50)) #(B, |A|)
            L_var_2 = ratio2 * q2_std / (q2_std_detach + bias) * (q2_std_detach**2 - optax.losses.huber_loss( jax.lax.stop_gradient(observed_q_values[1]), target_q_bound_2, 50)) # (B, |A|)
            
            q1_loss = L_mean_1.mean() + L_var_1.mean()
            q2_loss = L_mean_2.mean() + L_var_2.mean()
    ...
    def warmup_train_critic(actor_state, qf_state, batch_obs,
        ...
        normal_values = jax.random.normal(normal_key, mean_next.shape, dtype=jnp.float32) 
        normal_values = jnp.clip(normal_values,-3,3) *  jax.lax.stop_gradient(std_next) + jax.lax.stop_gradient(mean_next) #(2, |B|, |A|) 
        normal_values = (jnp.min(normal_values, axis = 0) * prob).sum(axis = -1)
        target_Z_values = jax.lax.stop_gradient(normal_values * (1-batch_dones) * discounts + batch_rewards)
        ...
        def mse_loss(params: flax.core.FrozenDict, dropout_key: jax.Array) -> jax.Array:
            ...
            observed_std = current_std_values[:,jnp.arange(batch_actions.shape[0]), batch_actions.astype(jnp.int32).squeeze(-1)] #(2, B)
            new_means_std = (1 - tau) * past_std + tau *  jax.lax.stop_gradient(observed_std).mean(axis = 1) # (2, |A|)

            td_bound = 3.0 *  jax.lax.stop_gradient(new_means_std)
            difference = jnp.clip(jax.lax.stop_gradient(target_Z_values) -  jax.lax.stop_gradient(observed_q_values), -td_bound[:, None], td_bound[:, None]) # (B,|A|)

            target_q_bound =  jax.lax.stop_gradient(observed_q_values) + difference # (B,|A|)

            loss = (0.5 * (target_Q_values - observed_q_values)**2).sum(axis = 1).mean() + (jax.lax.stop_gradient(new_means_std[:, None]**2) - (observed_q_values - target_q_bound)**2 ).mean(axis = 1).sum()
    ...
    def on_done_processor(tmp_buffer, env_rec, step_carry, latency_manager, reset_key, env):
