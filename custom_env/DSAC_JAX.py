...
class DSAC_JAX(SAC):
    ...
    def rollout_warmup(cls, env: EnvDataBinance, n_episodes, n_networks_training, actor_state, qf_state, ent_coef_state, act_dim, tau, batch_size, past_entropy, obs_dim, past_std,
    ...
    def learning_wrapper(cls, total_timesteps, collect_timesteps, buffer_shape, env, eval_env, buffer: CustomBufferLatency, obs_dim, act_dim, gradient_steps, discounts,
        ...
        def env_rec_loop(carry: SACarry, _):
            ...
            loss_rec = cls.update_loss_rec(loss_rec, qf_loss_value, actor_loss_value, ent_coef_loss_value, ent_coef_value, Q_mean, carry.entropy, std = jnp.zeros(3))
    ...
    def learn_jax(
        ...
        carry, loss_warmup = self.learning_wrapper(total_timesteps, self.train_freq.frequency, self.buffer.buffer.shape[1], self.env, self.env_rec, self.buffer, self.env.obs_length, self.env.action_length, self.gradient_steps, self.gamma,
                                 ...
                                 eval_freq, n_eval, log_interval, self.latency_manager, self.latency_manager_test, self.start_entropy, self.start_std)
    ...
    def update_critic(
        ...
        target_Z_values = target_Z_values - C1 #(B)
        target_Z_values = jax.lax.stop_gradient(rewards[:, None] + C2[:, None] * target_Z_values) #(B)
        ...
        def mse_loss(params: flax.core.FrozenDict, dropout_key: jax.Array) -> jax.Array:
            ...
            difference_1 = jnp.clip(jax.lax.stop_gradient(target_Z_values) -  jax.lax.stop_gradient(current_q_values[0]), -td_bound_1, td_bound_1) # (B,|A|)
            difference_2 = jnp.clip(jax.lax.stop_gradient(target_Z_values) -  jax.lax.stop_gradient(current_q_values[1]), -td_bound_2, td_bound_2) # (B,|A|)
            ...
            L_mean_1 = ratio1[jnp.arange(actions.shape[0]), actions.astype(jnp.int32).squeeze(-1)] * optax.losses.huber_loss(observed_q_values[0] , target_Q_values, 50) # (B)
            L_mean_2 = ratio2[jnp.arange(actions.shape[0]), actions.astype(jnp.int32).squeeze(-1)] * optax.losses.huber_loss(observed_q_values[1] , target_Q_values, 50) # (B)
            
            L_var_1 = ratio1 * q1_std / (q1_std_detach + bias) * (q1_std_detach**2 - optax.losses.huber_loss( jax.lax.stop_gradient(current_q_values[0]), target_q_bound_1, 50)) #(B, |A|)
            L_var_2 = ratio2 * q2_std / (q2_std_detach + bias) * (q2_std_detach**2 - optax.losses.huber_loss( jax.lax.stop_gradient(current_q_values[1]), target_q_bound_2, 50)) # (B, |A|)
    ...
    def warmup_train_critic(actor_state, qf_state, ent_coef_state, batch_obs,
        ...
        normal_values = jax.random.normal(normal_key, mean_next.shape, dtype=jnp.float32) 
        normal_values = jnp.clip(normal_values,-3,3) *  jax.lax.stop_gradient(std_next) + jax.lax.stop_gradient(mean_next) #(2, |B|, |A|) 
        target_Z_vector = normal_values * (1-batch_dones[:, None]) * discounts + batch_rewards[:, None]
