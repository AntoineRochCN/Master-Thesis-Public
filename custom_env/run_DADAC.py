...
if __name__ == '__main__':
    ...
    model = DADAC_JAX("DiscretePolicy", DISRESPECT_THE_ENV, env, env_test, latency_manager, latency_manager_test, buffer, tensorboard_log="./test/", replay_buffer_class=None,
                policy_kwargs=dict(net_arch=[256, 256, 256], activation_fn = jax.nn.gelu, optimizer_kwargs = {"eps": 1e-4}), learning_rate=1e-4, policy_delay=1, learning_starts= 10000//train_freq, buffer_size=1000000, tau = 0.005 ,
                train_freq=train_freq, seed=0, gradient_steps=1, action_noise=None, alpha_0=0.1, gamma=0.99, target_entropy=target_entropy, learning_rate_alpha=1e-4, batch_size=256)

    carry_out = model.learn_jax(10**6, eval_freq = 5000, n_eval = 10, log_interval=100, save_model=True, save_path="model_save_test.msgpack")
