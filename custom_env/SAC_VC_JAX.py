...
class SAC_VC_JAX(SAC):
    ...
    @staticmethod
    @partial(jax.jit, static_argnames = ["n_steps", "buffer_shape", "max_latency"], donate_argnames = ["env", "tmp_buffer", "env_rec"])
    def rollout(env: EnvDataBinance, n_steps, buffer_shape, actor_state, env_rec: EnvRecord, latency_manager: JaxLatencyEnv, tmp_buffer, buf_pos, buf_pos_init, max_latency):
        step_carry = env.step_carry
        ...
        def scan_body(items, keys_for_step):
            ...
            tmp_buffer = update_latency_buffer(tmp_buffer, tmp_buffer_pos, max_latency, buffer_shape, mask_return_arr, return_arr)
    ...
    def learning_wrapper(cls, total_timesteps, collect_timesteps, buffer_shape, env, eval_env, buffer: CustomBufferLatency, obs_dim, act_dim, gradient_steps, discounts,
        ...
        env, env_rec, latency_manager, tmp_buf, buf_pos, buf_pos_init = cls.rollout(env, init_timesteps, buffer_shape, actor_state, env_rec, latency_manager, buffer.buffer, buffer.pos, buffer.past_begin_pos, buffer.max_latency)
        ...
        def critic_loop(items, _):
            ...
            (
                ...
                key,
                                    ...
                                    buffer.max_latency, batch_latencies, 
                                    past_entropy)
            qf_state = cls.soft_update(tau, qf_state)
        ...
        def rollout_loop(items, _):
            ...
            env, env_rec, latency_manager, tmp_buf, buf_pos, buf_pos_init = cls.rollout(
                        ...
                        buffer.buffer, buffer.pos, buffer.past_begin_pos, buffer.max_latency)
    ...
    def learn_jax(
        ...
        t1 = time.time()
        carry, warmup_loss = self.learning_wrapper(total_timesteps, self.train_freq.frequency, self.buffer.buffer.shape[2], self.env, self.env_rec, self.buffer, self.env.obs_length, self.env.action_length, self.gradient_steps, self.gamma_arr,
        ...
        if self.tensorboard_log != None:
            tensorboard_logger(carry.env_rec, carry.loss_rec, carry.test_rec, self.tensorboard_log, tb_log_name, self.env.obs_length, log_interval = log_interval, 
                               collect_timesteps=self.train_freq.frequency, test_frequency=eval_freq, loss_warmup=warmup_loss)
    ...
    def update_critic(
        ...
        target_Q_values = (mean_next * prob[None, ...]).sum(axis = 3) # (2, B, T)
        target_Q_values = jnp.min(target_Q_values, axis=0) # (B, T)

        target_Q_values = target_Q_values - C1 #(B, T)
