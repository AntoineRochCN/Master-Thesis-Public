...
class SAC_VC_JAX(SAC):
    ...
    def update_critic(
        ...
        target_Q_values = ((jnp.min(mean_next, axis = 0) - C1) * prob).sum(axis = -1) # (B, T)
    ...
    def warmup_train_critic(actor_state, qf_state, batch_obs,
        ...
        target_Q_values = jax.lax.stop_gradient(target_Q_values * (1-batch_dones) * discounts[:, 1] + batch_rewards)
        ...
        def mse_loss(params: flax.core.FrozenDict, dropout_key: jax.Array) -> jax.Array:
            ...
            loss = (0.5 * (target_Q_values[None, ...] - observed_q_values)**2).sum(axis = 1).mean()
